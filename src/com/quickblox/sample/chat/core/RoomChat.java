...
public class RoomChat implements Chat, RoomListener, ChatMessageListener {
	...
	public RoomChat(ChatActivity chatActivity) {
		...
		String chatRoomName = chatActivity.getIntent().getStringExtra(
				EXTRA_ROOM_NAME);
		RoomAction action = (RoomAction) chatActivity.getIntent()
				.getSerializableExtra(EXTRA_ROOM_ACTION);
	...
	public void sendMessage(String message) throws XMPPException {
		if (chatRoom != null) {
			...
			Toast.makeText(chatActivity, "Join unsuccessful", Toast.LENGTH_LONG)
					.show();
		}
	}
	...
	public void processMessage(Message message) {
		...
		if (sender.equals(qbUser.getFullName())
			...
			chatActivity.showMessage(new ChatMessage(message.getBody(), time,
					false));
		} else {
	...
	/*
	 * public String nomeUtenteDaID(String sender){ QBUser user=
	 * QBUser.findById(sender) ; String utente= user.getLogin(); return utente;
	 * }
	 */
